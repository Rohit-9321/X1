...
export default function SignupPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    const result = await dispatch(signup({ fullName: form.fullName, email: form.email, phone: form.phone, password: form.password }));
    if (signup.fulfilled.match(result)) { toast.success('Account created! Welcome to X1 🎉'); navigate('/dashboard'); }
    else toast.error(result.payload || 'Signup failed');
  };
    ...
    <div className="min-h-screen flex items-center justify-center bg-surface px-4 py-10">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-ink mt-4">Create your account</h1>
          <p className="text-gray-500 text-sm mt-1">Start preparing for your dream company</p>
        </div>
        ...
        <div className="card">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {[['fullName','Full Name','text','John Doe'],['email','Email','email','you@email.com'],['phone','Phone (optional)','tel','+91 9876543210']].map(([f,l,t,ph]) => (
              <div key={f}>
                <label className="block text-sm font-medium text-gray-700 mb-1.5">{l}</label>
                <input className="input" type={t} placeholder={ph} required={f !== 'phone'}
                  value={form[f]} onChange={handle(f)} />
              </div>
            ))}
            {[['password','Password'],['confirm','Confirm Password']].map(([f,l]) => (
              <div key={f}>
                <label className="block text-sm font-medium text-gray-700 mb-1.5">{l}</label>
